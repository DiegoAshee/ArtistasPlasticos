...
public function createMultiple(): void
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_payment'])) {
        ...
        if ($idPaymentType <= 0) {
            ...
            if (isset($_POST['upload_receipt']) && !empty($_FILES['receipt_file']['name'])) {
                if (APP_DEBUG) {
                    echo "<div style='background: #e7f3ff; padding: 10px; margin: 10px 0; border: 1px solid #bee5eb;'>";
                    echo "DEBUG: Intentando subir archivo: " . $_FILES['receipt_file']['name'];
                    echo "</div>";
                }
                ...
                if ($uploadResult['success']) {
                    ...
                    if (APP_DEBUG) {
                        ...
                        echo "DEBUG: voucherImageURL set to: " . $voucherImageURL;
                        echo "</div>";
                    }
                } else {
                    $error = $uploadResult['error'];
                ...
                if (APP_DEBUG) {
                    ...
                    echo "DEBUG: No se subió archivo - upload_receipt: " . (isset($_POST['upload_receipt']) ? 'YES' : 'NO') . 
                         ", file name: " . ($_FILES['receipt_file']['name'] ?? 'EMPTY');
                    echo "</div>";
                }
            }
            ...
            if (!isset($error)) {
                ...
                foreach ($debtsData as $debt) {
                    if (APP_DEBUG) {
                        ...
                        echo "DEBUG: Creando pago - idPartner: {$debt['idPartner']}, idContribution: {$debt['idContribution']}, amount: {$debt['amount']}, voucherImageURL: " . ($voucherImageURL ?? 'NULL');
                        echo "</div>";
                    }
                    ...
                    if (APP_DEBUG) {
                        echo "<div style='background: " . ($paymentId ? '#d4edda' : '#f8d7da') . "; padding: 10px; margin: 10px 0;'>";
                        echo "DEBUG: Resultado create: " . ($paymentId ? $paymentId : 'FALSE');
                        echo "</div>";
                    }
                ...
                if ($successCount > 0) {
                    ...
                    // SI DEBUG ESTÁ ACTIVO, NO REDIRIGIR PARA VER MENSAJES
                    if (!APP_DEBUG) {
                        $this->redirect('cobros/recibo');
                        return;
                    } else {
                        ...
                        echo "<br><a href='" . u('cobros/recibo') . "'>IR AL RECIBO</a>";
                        echo "</div>";
                    }
                } else {
                    $error = 'No se pudo procesar ningún pago. ' . implode(', ', $errors);
...
private function handleReceiptUpload(array $file, int $partnerId): array
{
    if (APP_DEBUG) {
        ...
        echo "DEBUG handleReceiptUpload - partnerId: {$partnerId}<br>";
        echo "File data: ";
        print_r($file);
        echo "</div>";
    }
    ...
    // Crear directorio si no existe - USAR BASE_PATH en lugar de __DIR__
    $uploadDir = BASE_PATH . '/public/images/receipts/';
    
    if (APP_DEBUG) {
        echo "<div style='background: #fff3cd; padding: 10px; margin: 10px 0;'>";
        echo "DEBUG: Upload dir: " . $uploadDir . "<br>";
    ...
    if (APP_DEBUG) {
        ...
        echo "DEBUG: fileName: {$fileName}<br>";
